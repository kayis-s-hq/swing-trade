...
| AD-H4: BigDecimal precision loss | ✅ Done | Backtest engines, `BacktestTrade`, `SignalResult`, `BacktestConfig` money knobs, backtest/portfolio result money and percent fields and `PortfolioExposureContext` use `BigDecimal` (`FinancialScale`: 4 dp money/percent, 6 dp indicators). Sharpe, Sortino, Calmar, CAGR, winRate and expectancy stay `double` by design. Persisted `Double` columns (`CandidateScanResultEntity`, core `BacktestResult`) would need a migration. |
| AD-H5: Circular dependency between broker and data | ✅ Done | Module-level cycle was already gone (`ModuleBoundaryTest` enforces `beFreeOfCycles()`). The in-module bean cycle is removed: `PaperTradingEngine` depends on the `TradingStatePersistence` port in `core`, `PaperTradingStateService` implements it, no `@Lazy`/setter injection. |
| AD-H6: Position is a 23-field God Object | ✅ Done | `Position` is an aggregate of `PositionEntry`/`PositionRisk`/`PositionValuation`/`PositionExit`; the 23-arg compatibility constructor is removed. `GET /api/positions` and open-position stats are served from the `PositionSummary` projection with an unchanged JSON contract (pinned by `PositionListContractTest`). |
