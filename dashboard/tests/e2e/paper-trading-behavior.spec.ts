...
test.describe('Paper Trading Behavior', () => {
  ...
  test('API: positions endpoint returns open positions with valid structure', async ({ request }) => {
  ...
  test('API: close position endpoint does not crash (pre-existing 500 known)', async ({ request }) => {
  ...
  test('UI: positions page shows symbol names', async ({ page }) => {
    ...
    const hasSymbol = bodyText?.includes('RELIANCE') ||
