...
test.describe('Paper Trading Behavior', () => {
  test('API: positions endpoint returns open positions with valid structure', async ({
    ...
    // Open positions may be 0 if all are closed — check closed too
    const closedRes = await request.get(`${API}/api/positions/closed`)
    const closedData = (await closedRes.json()) as any
    const closedPositions = closedData.content ?? []
    const total = positions.length + closedPositions.length
    expect(total).toBeGreaterThan(0)
    const sample = [...positions, ...closedPositions][0]
    expect(sample).toHaveProperty('symbol')
    expect(sample).toHaveProperty('entryPrice')
  ...
  test('API: performance endpoint returns valid structure', async ({ request }) => {
    const res = await request.get(`${API}/api/positions/performance`)
    expect(res.status()).toBe(200)
  ...
  test('API: individual position by symbol returns 200', async ({ request }) => {
    const res = await request.get(`${API}/api/positions/RELIANCE`)
    expect(res.status()).toBe(200)
  ...
  test('API: close position endpoint does not crash (pre-existing 500 known)', async ({
    ...
    const res = await request.post(`${API}/api/positions/RELIANCE/close`, {
