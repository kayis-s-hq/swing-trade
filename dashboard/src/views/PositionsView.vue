  ...
  <div class="p-6 animate-fade-in">
    ...
    <div class="mb-6 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        ...
        <button @click="showNewPositionModal = true" class="flex items-center gap-2 rounded-md bg-brand px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-brand/90">
          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-border-subtle bg-bg-primary/50">
                ...
                <th class="px-5 py-3 text-center text-xs font-semibold uppercase tracking-wider text-text-muted">Action</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-border-subtle/50">
              <tr v-for="pos in filteredPositions" :key="pos.id" class="transition-colors hover:bg-bg-hover">
                ...
                <td class="px-5 py-4 text-center">
                  <button v-if="pos.status === 'OPEN'" @click="showCloseModal(pos)" class="rounded-md border border-danger/30 px-2.5 py-1 text-xs font-medium text-danger transition-colors hover:bg-danger/10">
                    Close
                  </button>
              ...
              <tr v-if="filteredPositions.length === 0">
                <td colspan="9" class="px-5 py-12 text-center text-sm text-text-muted">No positions found</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <!-- New Position Modal -->
    <Teleport to="body">
      <div v-if="showNewPositionModal" class="fixed inset-0 z-50 flex items-center justify-center" @click.self="showNewPositionModal = false">
        ...
        <div class="relative w-full max-w-md rounded-xl bg-bg-primary p-6 shadow-xl">
          <div class="mb-4 flex items-center justify-between">
            ...
            <button @click="showNewPositionModal = false" class="text-text-muted hover:text-text-primary">
          ...
          <form @submit.prevent="submitNewPosition" class="space-y-4">
            <div>
              <label class="mb-1 block text-xs font-medium text-text-muted">Symbol</label>
              <input v-model="newPos.symbol" required placeholder="e.g. RELIANCE" class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none" />
            </div>

            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="mb-1 block text-xs font-medium text-text-muted">Direction</label>
                <select v-model="newPos.direction" class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary focus:border-brand/50 focus:outline-none">
              ...
              <div>
                ...
                <select v-model="newPos.orderType" class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary focus:border-brand/50 focus:outline-none">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model.number="newPos.quantity" type="number" min="1" required class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none" />
              </div>
              <div>
                <label class="mb-1 block text-xs font-medium text-text-muted">Entry Price</label>
                <input v-model.number="newPos.price" type="number" step="0.01" min="0.01" required class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none" />
              </div>
            </div>
            ...
            <div v-if="newPos.orderType === 'LIMIT'" class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model.number="newPos.limitPrice" type="number" step="0.01" min="0.01" required class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model.number="newPos.stopLoss" type="number" step="0.01" min="0.01" placeholder="Optional" class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none" />
              </div>
              <div>
                <label class="mb-1 block text-xs font-medium text-text-muted">Target</label>
                <input v-model.number="newPos.target" type="number" step="0.01" min="0.01" placeholder="Optional" class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none" />
              </div>
            </div>

            <div>
              <label class="mb-1 block text-xs font-medium text-text-muted">Entry Reason (Optional)</label>
              <textarea v-model="newPos.entryReason" rows="2" placeholder="Why are you entering this trade?" class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none"></textarea>
            </div>

            <div class="flex gap-3">
              <button type="button" @click="showNewPositionModal = false" class="flex-1 rounded-md border border-border-subtle px-4 py-2 text-sm font-medium text-text-muted transition-colors hover:bg-bg-hover">Cancel</button>
              <button type="submit" :disabled="submitting" class="flex-1 rounded-md bg-brand px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand/90 disabled:opacity-50">
    ...
    <Teleport to="body">
      <div v-if="showClosePositionModal && closeTarget" class="fixed inset-0 z-50 flex items-center justify-center" @click.self="showClosePositionModal = false">
        <div class="absolute inset-0 bg-black/50"></div>
        <div class="relative w-full max-w-sm rounded-xl bg-bg-primary p-6 shadow-xl">
          <div class="mb-4 flex items-center justify-between">
            ...
            <button @click="showClosePositionModal = false" class="text-text-muted hover:text-text-primary">
              <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          ...
          <form @submit.prevent="submitClosePosition" class="space-y-4">
            <div>
              <label class="mb-1 block text-xs font-medium text-text-muted">Exit Reason (Optional)</label>
              <input v-model="closeReason" placeholder="e.g. Stop loss hit, target reached" class="w-full rounded-md border border-border-subtle bg-bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted/60 focus:border-brand/50 focus:outline-none" />
            </div>

            <div class="flex gap-3">
              <button type="button" @click="showClosePositionModal = false" class="flex-1 rounded-md border border-border-subtle px-4 py-2 text-sm font-medium text-text-muted transition-colors hover:bg-bg-hover">Cancel</button>
              <button type="submit" :disabled="closing" class="flex-1 rounded-md bg-danger px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-danger/90 disabled:opacity-50">
...
const submitNewPosition = async () => {
  ...
  try {
    ...
    newPos.value = { symbol: '', direction: 'LONG', orderType: 'MARKET', quantity: 1, price: undefined, limitPrice: undefined, stopLoss: undefined, target: undefined, entryReason: '' }
