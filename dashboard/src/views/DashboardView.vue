<template>
  <div class="p-6 animate-fade-in">
    ...
    <!-- Error -->
    <ErrorMessage v-else-if="error" :message="errorMessage" :showRetry="true" retryText="Retry" @retry="refreshDashboard" />
    ...
    <template v-else>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <div class="mt-2 flex items-baseline gap-3">
              <span class="text-3xl font-bold tracking-tight" :class="(marketOverview?.todayPnl ?? 0) >= 0 ? 'text-success' : 'text-danger'">
                {{ (marketOverview?.todayPnl ?? 0) >= 0 ? '+' : '' }}${{ (marketOverview?.todayPnl ?? 0).toLocaleString() }}
              </span>
              <span class="text-lg font-medium" :class="(portfolioSummary?.totalPnlPercent ?? 0) >= 0 ? 'text-success' : 'text-danger'">
      ...
      <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-5">
        <div v-for="metric in metrics" :key="metric.title" class="card-panel p-4">
          <p class="text-xs font-medium text-text-muted">{{ metric.title }}</p>
          <p class="mt-1 text-xl font-bold text-text-primary">{{ metric.value }}</p>
          <div v-if="metric.trend" class="mt-1 flex items-center gap-1">
            <span class="text-xs font-medium" :class="metric.trend.isPositive ? 'text-success' : 'text-danger'">
      ...
      <div class="card-panel">
        <div class="flex items-center justify-between border-b border-border-subtle px-5 py-3">
          ...
          <router-link to="/positions" class="text-sm font-medium text-brand hover:underline">View All →</router-link>
        </div>
        <div class="w-full overflow-x-auto">
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-border-subtle/50">
              <tr v-for="pos in positions.slice(0, 5)" :key="pos.id" class="transition-colors hover:bg-bg-hover">
...
const metrics = computed(() => [
  ...
  { title: 'P&L', value: `$${(portfolioSummary.value?.totalPnl ?? 0).toLocaleString()}`, trend: { value: `${(portfolioSummary.value?.totalPnlPercent ?? 0).toFixed(2)}%`, isPositive: Boolean(portfolioSummary.value?.totalPnlPercent && portfolioSummary.value.totalPnlPercent >= 0) } },
])
