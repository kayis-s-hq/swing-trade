  ...
  <div class="view-shell strategies-view p-4 sm:p-6 animate-fade-in">
    ...
    <ErrorBoundary :error="Boolean(store.error)">
      ...
      <template v-else>
        ...
        <!-- Compare -->
        <section class="card-panel mb-6 p-5">
          <h2 class="mb-4 text-lg font-semibold text-text-primary">Compare</h2>
          <p class="mb-3 text-xs text-text-muted">
            Selected: {{ selectedForCompare.length === 0 ? 'none' : selectedForCompare.join(', ') }}
          </p>
          <div class="mb-3 flex flex-wrap gap-3">
            <label class="flex flex-col gap-1 text-xs text-text-muted">
              Start
              <input v-model="compareStart" type="date" class="rounded-md border border-border-subtle bg-bg-primary px-2 py-1 text-sm text-text-primary" />
            </label>
            <label class="flex flex-col gap-1 text-xs text-text-muted">
              End
              <input v-model="compareEnd" type="date" class="rounded-md border border-border-subtle bg-bg-primary px-2 py-1 text-sm text-text-primary" />
            </label>
          ...
          <button
            ...
            :disabled="selectedForCompare.length === 0 || store.comparing"
            @click="handleCompare"
          >
          ...
          <div v-if="store.compareResult" class="mt-5 overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody>
                <template v-for="vr in store.compareResult.variants" :key="vr.variantId">
                  ...
                  >
                    <td class="py-2 pr-3 font-medium text-text-primary">{{ vr.variantId }} (v{{ vr.version }})</td>
                    <td class="py-2 pr-3">{{ fold.fold === 0 ? 'plain' : fold.fold }}</td>
                    <td class="py-2 pr-3">{{ fold.windowStart }} – {{ fold.windowEnd }}</td>
                    <td class="py-2 pr-3">{{ metricValue(fold.metrics, 'sharpe') }}</td>
                    <td class="py-2 pr-3">
                      {{ vr.deflatedSharpeRatio.toFixed(3) }}
                      <span v-if="vr.deflatedSharpeRatio > 0.1" class="ml-1 text-[11px]">(likely noise)</span>
                    </td>
                    <td class="py-2 pr-3">{{ metricValue(fold.metrics, 'tradeCount') }}</td>
            ...
            <p class="mt-3 text-xs text-text-muted">
              Overlaid equity curves and drawdown chart are not yet implemented for this view
              (deferred — see report). Signal overlap matrix is deferred pending a
              SignalOverlapAnalyzer endpoint, which does not currently exist.
            </p>
          </div>
        </section>
...
async function handleCompare() {
  await store.runCompare({
    variants: selectedForCompare.value.map((id) => ({ id })),
...
function metricValue(metrics: Record<string, unknown>, key: string): string {
  ...
  if (value === undefined || value === null) return 'not yet available'
  return typeof value === 'number' ? value.toFixed(3) : String(value)
}

function winRateWithCi(metrics: Record<string, unknown>): string {
  const winRate = metrics['winRate']
  const ciLow = metrics['winRateCiLow']
  const ciHigh = metrics['winRateCiHigh']
  if (winRate === undefined || winRate === null) return 'not yet available'
  if (ciLow === undefined || ciHigh === undefined || ciLow === null || ciHigh === null) {
    return 'not yet available (no CI provided)'
  }
