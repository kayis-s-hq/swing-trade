<template>
  <div class="view-shell p-4 sm:p-6 animate-fade-in">
    ...
    <p v-else-if="errorText" class="card-panel p-5 text-sm text-danger" role="alert">{{ errorText }}</p>
    ...
    <template v-else-if="report">
      <div class="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div class="card-panel p-4">
          ...
          <div class="text-xl font-semibold text-text-primary">{{ pct(report.totals.avgRegretPct) }}</div>
        </div>
      </div>
      ...
      <div class="card-panel mb-6 p-5" aria-label="Wins by variant">
        ...
        <div v-else class="space-y-2">
          <div v-for="v in report.variants" :key="v.variantId" class="flex items-center gap-3 text-sm">
      ...
      <div class="card-panel p-5" aria-label="Strategy leaderboard">
        ...
        <p v-if="report.variants.length === 0" class="text-sm text-text-muted">No active variants.</p>
        <div v-else class="overflow-x-auto">
          <table class="w-full min-w-[960px] text-sm">
            <thead>
              <tr class="border-b border-border-subtle text-left text-xs font-medium text-text-muted">
            ...
            <tbody>
              <tr v-for="v in report.variants" :key="v.variantId" class="border-b border-border-subtle/50">
                <td class="py-2 pr-3">
                  <div class="font-medium text-text-primary">{{ v.variantId }}</div>
                  <div class="text-xs text-text-muted">{{ v.strategyType }} · {{ v.mode }}</div>
                </td>
                <td class="py-2 pr-3 text-text-muted">{{ v.signalsGenerated }} / {{ v.buySignals }}</td>
                ...
                <td class="py-2 pr-3" :class="pnlClass(v.selectedPnl)">{{ money(v.selectedPnl) }}</td>
        ...
        <p class="mt-3 text-xs text-text-muted">
          Regret = gap between the best rejected BUY's shadow return and the winner's, only when both
          have closed trades. Agreement = share of other active variants also buying on the same day.
        </p>
      </div>
    </template>
  </div>
...
import { getStrategyReport, type StrategyReport } from '../api/strategyReport'
