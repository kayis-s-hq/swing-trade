<template>
  ...
  >
    ...
    <div class="h-[2px] w-full" :class="statusBarColor"></div>

    <!-- Header: Symbol + Status -->
    <div class="flex items-start justify-between border-b border-border-subtle/50 px-4 py-3">
      <div>
        <h3 class="text-sm font-bold tracking-tight text-text-primary">{{ position.symbol }}</h3>
        <p class="mt-0.5 text-[10px] text-text-muted">{{ position.reason }}</p>
      </div>
      <span
      ...
      >
        <span class="h-1 w-1 rounded-none" :class="statusDotColor"></span>
        {{ position.status }}
      </span>
    </div>
    ...
    <div class="flex items-center justify-between border-t border-border-subtle/50 px-4 py-3">
      <div>
        ...
        <p class="text-sm font-medium text-text-primary">{{ position.quantity }}</p>
      </div>
    ...
    <div class="pointer-events-none absolute -right-[1px] -top-[1px] h-3 w-3">
      <div class="absolute right-0 top-0 h-[1px] w-3 bg-brand/20 transition-all group-hover:bg-brand/50"></div>
      <div class="absolute right-0 top-0 h-3 w-[1px] bg-brand/20 transition-all group-hover:bg-brand/50"></div>
    </div>
  </div>
</template>
...
const statusBarColor = computed(() => {
  switch (props.position.status) {
    ...
    case 'TARGET_HIT': return 'bg-brand'
...
const statusBadgeClass = computed(() => {
  switch (props.position.status) {
    case 'OPEN': return 'border-success/30 bg-success-bg text-success'
    case 'STOPPED': return 'border-warning/30 bg-warning-bg text-warning'
    case 'TARGET_HIT': return 'border-brand/30 bg-brand-subtle text-brand'
    default: return 'border-border-default bg-bg-elevated text-text-muted'
  }
})
