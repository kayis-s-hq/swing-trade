    ...
    <template v-else>
      <div class="grid grid-cols-3 gap-3">
        <div class="rounded-md bg-bg-elevated px-3 py-2">
          ...
          <p class="text-lg font-semibold text-text-primary">{{ totalTrades }}</p>
        </div>
        <div class="rounded-md bg-bg-elevated px-3 py-2">
        ...
        <div class="rounded-md bg-bg-elevated px-3 py-2">
          ...
          <p class="text-lg font-semibold" :class="pfColor">{{ profitFactor.toFixed(2) }}</p>
        </div>
      </div>
...
const winRateColor = computed(() => winRate.value >= 55 ? 'text-success' : winRate.value >= 45 ? 'text-warning' : 'text-danger')
const pfColor = computed(() => profitFactor.value >= 1.5 ? 'text-success' : profitFactor.value >= 1.0 ? 'text-warning' : 'text-danger')
