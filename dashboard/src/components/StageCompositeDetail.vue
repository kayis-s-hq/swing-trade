<template>
  <div class="flex flex-col gap-3">
    ...
    <div
      v-if="synthesis"
      class="border-t border-border-subtle/50 pt-3"
    >
      ...
      <div class="mb-3 rounded-md bg-bg-elevated px-3 py-2">
        <div class="mb-1 flex items-center justify-between">
          <span
            ...
            :class="synthesisRecommendationColor"
      ...
      <div
        v-if="synthesis.keyDrivers.length"
      ...
      >
        ...
        <ul class="list-disc pl-4 text-xs text-text-secondary">
          <li
            v-for="d in synthesis.keyDrivers"
            :key="d"
          >
            {{ d }}
          </li>
      ...
      <div class="mt-2 grid grid-cols-2 gap-3">
        <div
          v-if="synthesis.bullishFactors.length"
          class="flex flex-col gap-1"
        >
          ...
          <ul class="list-disc pl-4 text-xs text-text-secondary">
            <li
              v-for="f in synthesis.bullishFactors"
              :key="f"
            >
              {{ f }}
            </li>
        ...
        <div
          v-if="synthesis.bearishFactors.length"
          class="flex flex-col gap-1"
        >
          ...
          <ul class="list-disc pl-4 text-xs text-text-secondary">
            <li
              v-for="f in synthesis.bearishFactors"
              :key="f"
            >
              {{ f }}
            </li>
...
const compositeConfidence = computed(
  () => (props.details.payload.compositeConfidence as number) ?? 0
)
